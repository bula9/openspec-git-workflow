---
name: "OPSX: Release"
description: "Archive a completed change, bump version with SemVer, merge to main, and push to GitHub"
allowed-tools: Bash(openspec:*, git:*), Edit, Read, Glob, Grep, AskUserQuestion
category: "Workflow"
tags: ["workflow", "archive", "release", "version", "git"]
---

Archive a completed change with automatic SemVer versioning, Git merge to main, tag, and push.

**Input**: Optionally specify a change name after `/opsx:release` (e.g., `/opsx:release add-auth`). If omitted, infer from conversation context or the current git branch. If vague or ambiguous, prompt for available changes.

**Steps**

1. **Run the standard archive workflow**

   Execute the full `/opsx:archive` workflow for the selected change. This handles:
...
**Output On Success (With Version Bump)**

```
## Release Complete

**Change:** add-flashcard-tag-filter
**Version:** 1.0.0 → 1.1.0 (MINOR)
**Reason:** proposal 中包含"新增词卡标签筛选功能"，spec delta 有 ADDED requirement，向后兼容。
**Archive:** openspec/changes/archive/2026-08-12-add-flashcard-tag-filter/
**Git:**
- ✅ Merged feature/add-flashcard-tag-filter → main
- ✅ Tagged v1.1.0
- ✅ Pushed to origin
- ✅ Branch deleted
```

**Output On Success (Version Skipped)**

If user chose "跳过版本更新", git operations still run (merge + push) but without version file changes or tag.

**Output On Error (Merge Conflict)**

```
## Release Blocked — Merge Conflict

**Change:** <change-name>
**Branch:** <branch> → main

Merge conflicts detected. Please resolve manually:
1. `git checkout main && git merge <branch>`
2. Resolve conflicts in the files listed above
3. `git commit` and `git push`
4. Then manually tag: `git tag -a "v<version>" -m "..."` and `git push --tags`
```
